        ...
        {!collapsed && (
          ...
          >
            ...
            <div className="flex items-center justify-between border-b border-border-soft bg-surface-deep px-4 py-3">
              <span className="text-[13px] font-semibold text-text-strong">Conversaciones</span>
              <button
                onClick={onNewChat}
            ...
            <div className="flex-1 overflow-y-auto p-2 space-y-1">
              {conversations.length === 0 ? (
                <div className="flex flex-col items-center gap-2 py-10 text-center">
                  <MessageSquare size={28} className="text-text-faint" />
                  <p className="text-[12px] text-text-faint leading-snug px-4">
                ...
                conversations.map((conv) => (
                  ...
                  >
                    <div className="flex items-start justify-between gap-2">
                      <p className="line-clamp-2 text-[12.5px] font-medium text-text-strong leading-snug flex-1">
                        {conv.title}
                      </p>
                      {hovered === conv.id && (
                        <button
                          ...
                          className="shrink-0 rounded-md p-0.5 text-text-faint hover:text-red-400 transition"
                          title="Eliminar"
                        >
                          <Trash2 size={13} />
                        </button>
